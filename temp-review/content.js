...
function scrapeAssignmentsFromDOM() {
  ...
  for (const r of rows) {
    ...
    // "Submission" in an <a> tag = submitted (exclude it)
    const submissionCol = cols[3]?.innerHTML || '';
    const noSubmission = /no\s+submission/i.test(submissionCol);
    // Check if there's a download link for "Submission" (submitted assignments have <a>...Submission</a>)
    const hasSubmission = /<a[^>]*>[\s\S]*?Submission[\s\S]*?<\/a>/i.test(submissionCol);
    const isSubmitted = hasSubmission && !noSubmission;

    // Debug log for troubleshooting
    console.log(`Assignment: ${title}`);
    console.log(`  Submission HTML: ${submissionCol.substring(0, 200)}`);
    console.log(
      `  noSubmission: ${noSubmission}, hasSubmission: ${hasSubmission}, isSubmitted: ${isSubmitted}`
    );
